...
pop p_eq[NY+2][NX+2], p_diff[NY+2][NX+2],forcing[NY+2][NX+2];
//Warning: The force term below has the units of acceleration
...
void compute_feq()
{
  ...
  for (int y=1; y<NY+1; y++) {
    for (int x=1; x<NX+1; x++) {
      ...
      rho = m(p[y][x]);
//Shifting the velocity using GUO method
      u = (vx(p[y][x]) / rho) + 0.5*force[0];
      v = (vy(p[y][x]) / rho) + 0.5*force[1];
      usq = u * u;
      vsq = v * v;
...
void computeForcing(){
	...
	for (int y=1; y<NY+1; y++) {
    	for (int x=1; x<NX+1; x++) {
      		...
      		for(int q=0;q<9;q++){
      			forcing[y][x].p[q] = /*(1.0 - 0.5*invtau)*/w[q]*rho*(((cx[q] - u)*force[0] + (cy[q] - v)*force[1])/cs2 + ((cx[q]*u + cy[q]*v)*(force[0]*cx[q] + force[1]*cy[q]))/cs4);
      		}
	...
	for(int y=1;y<NY+1;y++)
	{
		for(int x=1;x<NX+1;x++)
		{
			for(int pp=0;pp<9;pp++)
			{
				...
				//p[y][x].p[pp] = p_eq[y][x].p[pp] + ((1.0 - invtau)*phatNew[y][x].p[pp]) + forcing[y][x].p[pp];//Corresponds to implementation #2
				//p[y][x].p[pp] = p_eq[y][x].p[pp] + ((1.0 - invtau)*phat[y][x].p[pp])+ forcing[y][x].p[pp];//Corresponds to implementation #1
				p[y][x].p[pp] = p[y][x].p[pp] - invtau * (p[y][x].p[pp] - p_eq[y][x].p[pp]) + forcing[y][x].p[pp];//Without using the regularization
			}
		}
	}	 
...
void write_pop(int tstep)
{
  ...
  fout = fopen(fname,"w");
    ...
    for (y=1; y<NY+1; y++){
      double tvx,tvy;
      int xcoord = 10;
      tvx = (vx(p[y][xcoord])/m(p[y][xcoord]) + 0.5*force[0]);
      tvy = (vy(p[y][xcoord])/m(p[y][xcoord]) + 0.5*force[1]);  
      fprintf(fout,"%d %d %g\n", xcoord, y, tvx);
	      }
    fprintf(fout,"\n");
